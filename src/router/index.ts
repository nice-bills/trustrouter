...
export function computeTrustScore(agent: AgentData): number {
    const reputationScore = Math.min(100, Math.max(0, agent.avgScore || 0));

    const validationScore = Math.min(100, Math.max(0, agent.validationAvg || 0));

    const activityScore = agent.feedbackCount > 0
        ? Math.min(100, 25 * Math.log10(agent.feedbackCount + 1))
        : 0;

    // 50% Reputation, 30% Validation, 20% Activity
    return Math.round((0.5 * reputationScore + 0.3 * validationScore + 0.2 * activityScore) * 100) / 100;
}
